...
pub fn submit_ei_to_all_nodes(client_id: ClientId, request: ClientRequest) {
    for node_index in 0..MAX_NODES {
        submit_impl(node_index, client_id, request.clone());
    }
}

pub async fn await_ei_response(node_index: usize) -> (ClientId, ClientResponse) {
    await_impl(node_index).await
}

#[cfg(feature = "channel-external-interface")]
fn submit_impl(node_index: usize, client_id: ClientId, request: ClientRequest) {
    CLIENT_REQUEST_HUB.send_request(node_index, client_id, request);
}

#[cfg(feature = "udp-external-interface")]
fn submit_impl(node_index: usize, client_id: ClientId, request: ClientRequest) {
    send_udp_ei_request(node_index, client_id, request);
}

#[cfg(feature = "channel-external-interface")]
async fn await_impl(node_index: usize) -> (ClientId, ClientResponse) {
    CLIENT_REQUEST_HUB.receive_response(node_index).await
}

#[cfg(feature = "udp-external-interface")]
async fn await_impl(node_index: usize) -> (ClientId, ClientResponse) {
    receive_udp_ei_response(node_index).await
}
