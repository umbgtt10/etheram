...
type RequestChannel =
    Channel<CriticalSectionRawMutex, (ClientId, RaftClientRequest), REQUEST_CAPACITY>;

type ResponseChannel =
    Channel<CriticalSectionRawMutex, (ClientId, RaftClientResponse), RESPONSE_CAPACITY>;
...
impl ClientRequestHub {
    ...
    pub fn try_receive_request(&self, node_index: usize) -> Option<(ClientId, RaftClientRequest)> {
    ...
    pub fn send_request(&self, node_index: usize, client_id: ClientId, request: RaftClientRequest) {
        let _ = self.requests[node_index].try_send((client_id, request));
        let _ = EI_NOTIFY_CHANNELS[node_index].try_send(());
    }

    pub async fn receive_response(&self, node_index: usize) -> (ClientId, RaftClientResponse) {
