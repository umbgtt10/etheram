...
fn compute_hash_different_height_returns_different_hash() {
    // Arrange
    let block_zero = Block::new(0, 1, vec![], [0u8; 32], BLOCK_GAS_LIMIT);
...
fn compute_hash_different_transactions_returns_different_hash() {
    ...
    let right_block = Block::new(5, 4, vec![tx_b, tx_a], [6u8; 32], BLOCK_GAS_LIMIT);
...
fn compute_hash_different_post_state_root_returns_different_hash() {
    // Arrange
    let mut block_a = Block::new(0, 1, vec![], [0u8; 32], BLOCK_GAS_LIMIT);
    let mut block_b = Block::new(0, 1, vec![], [0u8; 32], BLOCK_GAS_LIMIT);
